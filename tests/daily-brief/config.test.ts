...
test("defaults keep tech-news disabled and market enabled", () => {
  ...
  assert.equal(config.techNewsWindowHours, 30);
});
...
test("TECH_NEWS_ENABLED flag works without BRIEF_MODULES", () => {
  const config = loadDailyBriefConfig({
    ...
    TECH_NEWS_LIMIT: "3",
    TECH_NEWS_WINDOW_HOURS: "36",
  });
  assert.equal(config.techNewsEnabled, true);
  assert.equal(config.techNewsLimit, 3);
  assert.equal(config.techNewsWindowHours, 36);
});

test("clamps tech news limit and window to allowed ranges", () => {
  const config = loadDailyBriefConfig({
    TECH_NEWS_LIMIT: "99",
    TECH_NEWS_WINDOW_HOURS: "1",
  });
  assert.equal(config.techNewsLimit, 5);
  assert.equal(config.techNewsWindowHours, 24);
});
